...
The Ninth is now avaliable. It adds a window that can use the SDRs to send data, but the SoapySDR Library Routines do not seem to work on Windows. It has a prebuild exe for macOS, a install package for Windows and a zipped folder to unpack and run from for Ubuntu 18.04.
