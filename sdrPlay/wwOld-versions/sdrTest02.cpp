...
public:
	...
	void process3(int n);
	void process4(int n);
	std::atomic_int doWhat1;
	std::atomic_int doWhat2;
	std::atomic_int doWhat3;
	std::atomic_int doWhat4;
	struct playData *rx;
	struct Filters f;
...
void cDemod::process2(int n)
{
	while(1)
	{
		 ...
		 case Work:
			...
			printf("process2 work doWhat2 %d \n",doWhat2.load());
			...
			msresamp_crcf_execute(f.iqSampler, (liquid_float_complex *)buf2, rx->size, (liquid_float_complex *)buf, &num);  // decimate
		 	...
		 	doWhat3.store(Work);
			printf("process2 work done doWhat2 %d\n",doWhat2.load());
