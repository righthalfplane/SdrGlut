...
int Radio::dialogTimeIQ()
{
    ...
    record1time=rq.stop[0]/60;
    record2time=rq.stop[1]/60;
    record3time=rq.stop[2]/60;
    ...
    record1 = *localtime(&rq.start[0]);
    record2 = *localtime(&rq.start[1]);
    record3 = *localtime(&rq.start[2]);

/*
    printf("rq.start[0] %ld\n",rq.start[0]);
...
*/
    ...
    filePath[0] =
    glui->add_edittext_to_panel(panel3, "Path:", GLUI_EDITTEXT_TEXT, rq.FilePath[0]);
    filePath[0]->w=200;
    ...
    if(rq.frequency[0] == 0){
        ...
        rq.frequency[0]=rx->f/1e6;
        mstrncpy(rq.mode[0],Mode_Names[rx->decodemode],sizeof(rq.mode[0]));
    }
    
    msprintf(freq[0],sizeof(freq[0]),"%g",rq.frequency[0]);
    ...
    mode[0] =
    glui->add_edittext_to_panel(panel3, "Mode:", GLUI_EDITTEXT_TEXT, rq.mode[0]);
    mode[0]->w=200;
    ...
    filePath[1] =
    glui->add_edittext_to_panel(panel3, "Path:", GLUI_EDITTEXT_TEXT, rq.FilePath[1]);
    filePath[1]->w=200;
    
    
    new GLUI_Button(panel3, "Set Record File", 101, control_cb);
    
    msprintf(freq[1],sizeof(freq[0]),"%g",rq.frequency[1]);
    ...
    mode[1] =
    glui->add_edittext_to_panel(panel3, "Mode:", GLUI_EDITTEXT_TEXT, rq.mode[1]);
    mode[1]->w=200;
    ...
    filePath[2] =
    glui->add_edittext_to_panel(panel3, "Path:", GLUI_EDITTEXT_TEXT, rq.FilePath[2]);
    filePath[2]->w=200;
    
    
    new GLUI_Button(panel3, "Set Record File", 102, control_cb);
    
    msprintf(freq[2],sizeof(freq[0]),"%g",rq.frequency[2]);
    ...
    mode[2] =
    glui->add_edittext_to_panel(panel3, "Mode:", GLUI_EDITTEXT_TEXT, rq.mode[2]);
    mode[2]->w=200;
...
static int getValues()
{
  ...
  //  printf("%s %s %s %s %s\n",edit[2]->get_text(),edit[5]->get_text(),edit[8]->get_text(),edit[11]->get_text(),edit[14]->get_text());
    ...
    sdr->rq.start[0]=mktime(&record1);
    
    sdr->rq.start[1]=mktime(&record2);
    
    sdr->rq.start[2]=mktime(&record3);

    printf("start[0] %ld start[1] %ld start[2] %ld\n",sdr->rq.start[0],sdr->rq.start[1],sdr->rq.start[2]);

    sscanf(edit[16]->get_text(),"%ld", &sdr->rq.stop[0]);
    sdr->rq.stop[0] *= 60;
    sscanf(edit[17]->get_text(),"%ld", &sdr->rq.stop[1]);
    sdr->rq.stop[1] *= 60;
    sscanf(edit[18]->get_text(),"%ld", &sdr->rq.stop[2]);
    sdr->rq.stop[2] *= 60;

    
    sscanf(frequency[0]->get_text(),"%lf", &sdr->rq.frequency[0]);
    sscanf(frequency[1]->get_text(),"%lf", &sdr->rq.frequency[1]);
    sscanf(frequency[2]->get_text(),"%lf", &sdr->rq.frequency[2]);
