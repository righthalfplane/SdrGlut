...
// c++ -std=c++11 -o sendFFT01 sendFFT01.cpp -lSoapySDR -lsndfile -lliquid -Wall -Wno-return-type-c-linkage
// sendFFT01 "driver=hackrf"
// sendFFT01 "driver=bladerf"
#include <stdio.h>
	...
	while(loop){
		...
		if ((readcount = sf_readf_float (infile, buf, BLOCK_SIZE)) > 0){
			...
			for(int n=0;n<readcount;++n){
			 ...
			 //doWindow(buf,readcount,FILTER_BLACKMANHARRIS7);
			
/*
			double dx=1.0/freqAudio;
			printf("plot %d signal\n",readcount/100);
			for(int n=0;n<readcount/100;++n){
				double v=buf[n*2];
				double t=(n*dx);
				printf("%f %f\n",t,v);
			}
			...
			for(int n=0;n<readcount;++n){
				buf2[n*2]=buf[n*2];
				buf2[n*2+1]=buf[n*2+1];
			}
			
			si->forward_fft(buf2,readcount,FILTER_BLACKMANHARRIS7);
			    
			dx=freqAudio/(readcount-1);
			printf("plot %d FFT\n",readcount);
			for(int n=0;n<readcount;++n){
				double v=sqrt(buf2[n*2]*buf2[n*2]+buf2[n*2+1]*buf2[n*2+1]);
				double t=(n*dx);
				printf("%f %f\n",t,v);
			}
...
/*
			fprintf(stderr,"readcount %d num %d\n",readcount,num);
			
			for(int n=0;n<num;++n){
				buf[n*2]=buf2[n*2];
				buf[n*2+1]=buf2[n*2+1];
			}
			
			si->forward_fft(buf,4096,FILTER_BLACKMANHARRIS7);
			    
			double dx=freqSSB/(4096-1);
			printf("plot %d FFT\n",4096);
			for(int n=0;n<4096;++n){
				double v=sqrt(buf[n*2]*buf[n*2]+buf[n*2+1]*buf[n*2+1]);
				double t=(n*dx);
				printf("%f %f\n",t,v);
			}
