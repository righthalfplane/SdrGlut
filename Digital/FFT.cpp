...
int doFFT3D(double *x,double *y,long xsize,long ysize,long zsize,int direction,int filter)
{
	...
	if(!ifound){
	    fprintf(stderr,"doFFT3D Did not find power of 2 xsize %ld\n",xsize);
	    return 1;
	}
	...
	if(ysize > 1){
		...
		if(!ifound){
		    fprintf(stderr,"doFFT3D Did not find power of 2 ysize %ld\n",xsize);
		    return 1;
		}
		nd=2;
		if(zsize > 1){
			...
			if(!ifound){
			    fprintf(stderr,"doFFT3D Did not find power of 2 zsize %ld\n",xsize);
			    return 1;
			}
			nd=3;
		}else{
	...
	data=(struct complex2 *)malloc(length*sizeof(struct complex2));
	if(!data){
	    fprintf(stderr,"doFFT3D out of Memory\n");
	    return 1;
	}
...
int doFFT2(double *x,double *y,long length,int direction)
{
	...
	datar=(double *)malloc(2*length*sizeof(double));
	if(!datar){
